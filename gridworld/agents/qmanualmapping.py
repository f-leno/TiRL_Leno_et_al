...
class QManualMapping(QBaseTL):
    ...
    def __init__(self, seed=12345,alpha=0.9):        
        super(QManualMapping, self).__init__(seed=seed,alpha=alpha)
    ...
    def readQTable(self,state,action):
        ...
        qValue = 0.0
        if state != (('t',0,0)) and not self.updating:
        ...
        return self.qTable.get((state,action),0.0) + qValue
    ...
    def observe_reward(self,state,action,statePrime,reward):
        ...
        self.updating = True
        super(QManualMapping,self).observe_reward(state,action,statePrime,reward)
        self.updating = False
    def finish_episode(self):
        super(QManualMapping,self).finish_episode()
